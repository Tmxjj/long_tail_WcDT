...
class TrainModelTask(BaseTask):
    ...
    def execute(self, result_info: LoadConfigResultDate, task_logger:TaskLogger):
        ...
        # ours Dataset
        with open('data/train_data_all.json', 'r', encoding='utf-8') as file:  
        ...
        for epoch in range(train_model_config.num_epoch):
            self.fit_one_epoch(epoch, epoch_step, optimizer, model, data_loader, result_info,task_logger)
            scheduler.step()
            if (epoch+1) % 10 ==0:
                output_pth_path = result_info.task_config.model_dir + f'/epoch_{epoch+1}_model.pth'
                torch.save(model.state_dict(), output_pth_path)
                task_logger.logger.info(f'Epoch {epoch + 1}/{train_model_config.num_epoch} save as  + output_pth_path')
    ...
    def fit_one_epoch(
        ...
        for iteration, data in enumerate(data_loader):
            ...
            optimizer.zero_grad()
            diffusion_loss, traj_loss, confidence_loss, min_loss_traj,traj,confidence = model(data)
            ...
            pbar.update()
            task_logger.logger.info(f"Epoch {epoch_num + 1}/{train_model_config.num_epoch} | Batch {iteration +1} ———— total_loss: {total_losses / (iteration + 1)} diffusion_loss: {diffusion_losses / (iteration + 1)} traj_losses: {traj_losses / (iteration + 1)} confidence_losses:{ confidence_losses / (iteration + 1)}")
            # save_path = os.path.join(result_info.task_config.image_output_reuslts_dirdir,
            #                             f"epoch_{epoch_num}_pred.pt")
