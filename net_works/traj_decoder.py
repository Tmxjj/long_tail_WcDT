...
class TrajDecoder(nn.Module):
    def __init__(self, multimodal: int = 10, dim: int = 256, future_step: int = 10):
...
class Trajlongtail_Decoder(nn.Module):
    def __init__(self, multimodal: int = 10, dim: int = 256*75, future_step: int = 50):
        super(Trajlongtail_Decoder, self).__init__()
        self.multimodal = multimodal
        self.future_step = future_step
        self.one_modal = (future_step * 3 + 1)
        output_dim = multimodal * self.one_modal
    ...
    def forward(self, input_x):
        ...
        decoder_output = decoder_output.view(batch_size, self.multimodal, self.one_modal)
        traj = decoder_output[:, :, :-1].view(batch_size, self.multimodal, self.future_step, 3)
        confidence = decoder_output[:, :, -1]
        return traj, confidence