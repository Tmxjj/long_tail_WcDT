...
class DitDiffusionModel(nn.Module):
    def __init__(
            ...
            his_stp: int = 75, num_dit_blocks: int = 4
    ):
    ...
    def forward(self, perturbed_x, t, predicted_his_traj):
        ...
        # obs_num = perturbed_x.shape[1]
        # batch, his_stp * 2
        input_tensor = torch.flatten(perturbed_x, start_dim=1)
        # batch,  1
        t = t.view(-1,  1)
        # batch, his_stp * 2
        predicted_his_traj = torch.flatten(predicted_his_traj, start_dim=1)
        # batch, his_stp * 2 + 1
        ...
        return noise_output.view(batch_size, self.his_delt_step, -1)

# 用于扰动和去噪轨迹预测的高斯扩散过程
class GaussianDiffusion(nn.Module):
    def __init__(
            # his_stp
            self, input_dim: int = 2, conditional_dim: int = 2,
            his_stp: int = 75, betas: np.ndarray = None,
            loss_type: str = "l2", num_dit_blocks: int = 4,
            diffusion_type: str = "dit"
    ):
        ...
        self.input_dim = input_dim
        # nn.BatchNorm2d 是 PyTorch 中用于二维批归一化的模块。它通常用于卷积神经网络（CNN）中，
        # 对 4D 输入（例如 [batch_size, num_channels, height, width]）进行归一化,对num_channels进行归一化

        self.norm_output = nn.BatchNorm2d(5)

    def remove_noise(self, noise, t_batch, predicted_his_traj):
    #使用当前噪声、时间步和预测的历史轨迹作为输入，通过扩散模型计算出当前噪声的噪声成分 model_output。
    ...
    def sample(self, noise, predicted_his_traj): #反向扩散过程从噪声中生成无噪声的轨迹
        if self.diffusion_type != "none":
            ...
            # noise = torch.transpose(noise, 1, -1) # 将第一维度和最后一个维度交换
    ...
    def get_losses(self, predicted_his_traj_delt, predicted_his_traj, t):
        if self.diffusion_type != "none":
            noise = torch.randn_like(predicted_his_traj_delt)
            perturbed_x = self.perturb_x(predicted_his_traj_delt, t, noise)  # (128,74,2)
            ...
            diffusion_loss =  torch.sum(diffusion_loss)
            return diffusion_loss
        else:
            return torch.tensor(0).to(torch.float32)

    def forward(self, data: dict): #add noise
        ...
        return self.get_losses(predicted_his_traj_delt, predicted_his_traj, t)
