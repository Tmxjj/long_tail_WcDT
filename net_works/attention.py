...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, input_feature, attn_mask=None):
        batch_size,obe_num  = input_feature.shape[0],input_feature.shape[1]
        ...
        if input_feature.dim() == 2:
            ...
            out = out.transpose(1, 2).contiguous().reshape(batch_size, self.embed_dim)
            ...
            # Transpose back to (batch_size, seq_len, heads, head_dim)
            out = out.transpose(1, 2).contiguous().reshape(batch_size, obe_num, self.embed_dim)
        # Final linear transformation
        else:
            raise Exception(f"the dim of feature imput is error {input_feature.dim()}")
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, input_q, input_kv, attn_mask=None):
        batch_size , obe_num= input_q.shape[0],input_q.shape[1]
        # Linear transformations
        ...
        # Final linear transformation
            ...
            raise Exception(f"the dim of feature imput is error {input_q.dim()}")
